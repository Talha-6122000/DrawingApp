...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Setting the drawbale to our current selected color Pallet
        ...
        //OnClickListener on Brush Button
        ib_brush.setOnClickListener {
            showBrushSizeChooserDialog()
        }
        //On Gallery Button Click
        ib_gallery.setOnClickListener {
         //As the app required some permission to read the external data
         //to get the picture from gallery or Photos we need to see if user has allowed us to
            //to read the storage so let's check
            if (isReadStorageAllowed()){
                //Now if the read storage is allowed we will enter this code block
                //We will use implicit intent to interact with the gallery or photos app
                //to read the photo that user wants to select
    ...
    private fun requestStoragePermission() {
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(
            ...
            Toast.makeText(this,"Can't access pictures since the permission denied",Toast.LENGTH_SHORT).show()
    ...
    //This funcction will be called whenever the user click on the brush icon/Button
    //We need a variable and intialize it with the Dialog class
    //then set the contentview to the layout that we want to display as a dialog
    //then we accessd all the sizes of button and set the Brush size using the function
    // @setSizeForBrush() that is declared in DrawingVIEW
